...
* never break a line immediately before an _en_--dash, which would cause a
  distraction after the pause from refocusing after the line break;

* join a sequence footnote references to their preceding word, so the
